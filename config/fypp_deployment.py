...
C_PREPROCESSED = (
    "quadprog_linalg_constants" ,
    "quadprog_linalg_blas" ,
    "quadprog_linalg_lapack",
    "test_blas_lapack"
)
...
def pre_process_fypp(args):
    ...
    # Check destination folder for preprocessing. if not 'quadprog-fpm', it is assumed to be the root folder.
...
def deploy_quadprog_fpm(with_ilp64):
    """create the quadprog-fpm folder for backwards compatibility (to be deprecated)
    """
    import shutil
    prune=(
    ...
    if with_ilp64:
        base_folder = 'quadprog-fpm-ilp64'
    else:
        base_folder = 'quadprog-fpm'        
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Preprocess quadprog source files.')
    # fypp arguments
    ...
    parser.add_argument("--deploy_quadprog_fpm",action='store_true', help="create the quadprog-fpm folder")    
    ...
    if args.deploy_quadprog_fpm:
        deploy_quadprog_fpm(args.with_ilp64)
    #==========================================
    # build using fpm
    if args.build:
