...
contains
   ...
   subroutine collect_test_problems(testsuite)
      ...
      testsuite = [ &
                  ...
                  new_unittest("(Legacy) Inequality Constrained QP problem", test_legacy_problem_2), &
                  new_unittest("(Legacy/Compact) Inequality Constrained QP problem", test_legacy_compact_problem_2), &
                  new_unittest("Equality Constrained QP problem", test_problem_3), &
                  new_unittest("(Legacy) Equality Constrained QP problem", test_legacy_problem_3), &
                  new_unittest("(Legacy/Compact) Equality Constrained QP problem", test_legacy_compact_problem_3) &
                  ]
   end subroutine
   ...
   subroutine test_legacy_compact_problem_2(error)
      type(error_type), allocatable, intent(out) :: error
      integer, parameter :: n = 3
      ! Size of the problem.
      real(dp) :: P(n, n), q(n), C(2, n), d(n), obj
      real(dp), allocatable :: x(:), y(:)
      real(dp), allocatable :: work(:)
      integer               :: i, neq, ncons, r, lwork, nact, iter(2), info
      integer, allocatable  :: iact(:), iamat(:, :)
      ...
      allocate (iamat(3, 3)); iamat = 0
      iamat(1, 1) = 2; iamat(1, 2) = 2; iamat(1, 3) = 2
      iamat(2, 1) = 1; iamat(2, 2) = 1; iamat(2, 3) = 2
      iamat(3, 1) = 2; iamat(3, 2) = 2; iamat(3, 3) = 3
   ...
   subroutine test_legacy_problem_3(error)
      type(error_type), allocatable, intent(out) :: error
      ...
      !> Constrained solution.
      call check(error, norm2(xref - x) < 1e-6_dp, "Constrained solution is not correct.")
      if (allocated(error)) return
      !> Lagrange multipliers.
      call check(error, norm2(yref - y) < 1e-6_dp, "Lagrange mutilpliers are not correct.")
      if (allocated(error)) return
      !> Objective value.
      call check(error, abs(obj_ref - obj) < 1e-6_dp, "Minimum cost is not correct.")
      if (allocated(error)) return
      return
   end subroutine

   subroutine test_legacy_compact_problem_3(error)
      type(error_type), allocatable, intent(out) :: error
      integer, parameter :: n = 5, m = 3
      ! Size of the problem.
      real(dp) :: P(n, n), q(n), obj
      ...
      A(1, :) = [3.6_dp, -1.9_dp, -9.72_dp]
      A(2, :) = [-3.4_dp, -1.7_dp, -8.67_dp]
      A(3, :) = [-3.8_dp, -4.0_dp, 0.0_dp]
      A(4, :) = [1.6_dp, -4.0_dp, 0.0_dp]
      A(5, :) = [1.6_dp, 0.0_dp, 0.0_dp]

      allocate (iamat(n + 1, m)); iamat = 0
      iamat(1, 1) = 5; iamat(1, 2) = 4; iamat(1, 3) = 2
      iamat(2, 1) = 1; iamat(2, 2) = 2; iamat(2, 3) = 1
      iamat(3, 1) = 2; iamat(3, 2) = 3; iamat(3, 3) = 2
      iamat(4, 1) = 3; iamat(4, 2) = 4; iamat(4, 3) = 0
      iamat(5, 1) = 4; iamat(5, 2) = 5; iamat(5, 3) = 0
      iamat(6, 1) = 5; iamat(6, 2) = 0; iamat(6, 3) = 0
